...
__attribute__ ((section ("prekernel_entry"))) void prekernel_main(void)
{
    ...
    num_SMAP_entries = *(uint32_t *)SMAP_ENTRIES_NUM;
    SMAP_entry       = (SMAP_entry_t *)SMAP_ENTRIES;
    ...
    deinitialize_memory_region(0x0000, 0x11000);                            // Reserve all memory below C000h for the kernel/OS
    ...
    superblock->root_inode_pointer = BOOTLOADER_FIRST_INODE_ADDRESS + sizeof(inode_t);
    rw_sectors(8, superblock->first_data_block*8, CURRENT_DIR_ADDRESS, READ_WITH_RETRY);

    // Find kernel id
    dir_entry_t *dir_entry = (dir_entry_t *)CURRENT_DIR_ADDRESS;
    while (dir_entry->name[0] != '\0' && strncmp(dir_entry->name, "kernel.bin", 10) != 0) 
        dir_entry++;

    // Find kernel inode
    inode_t *inode = (inode_t *)BOOTLOADER_FIRST_INODE_ADDRESS;
    while (inode->id != dir_entry->id) 
        inode++;
